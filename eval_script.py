...
def evaluate_results(results_file, ground_truth_file=None):
    try:
    ...
    if ground_truth_file:
        try:
            with open(ground_truth_file, "r") as f:
                ground_truth_rows = json.load(f)
            for row in ground_truth_rows:
                if isinstance(row, dict) and row.get("id") is not None:
                    ground_truth_by_id[str(row.get("id"))] = row.get("expected_standards", [])
        except Exception as e:
    ...
    for item in data:
        ...
        expected_source = item.get("expected_standards", [])
        if ground_truth_by_id:
            expected_source = ground_truth_by_id.get(str(item.get("id")), [])
        expected = set(normalize_std(std) for std in expected_source)
        retrieved_source = item.get("retrieved_standards", [])
        retrieved = [normalize_std(std) for std in retrieved_source]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to the labeled test set for scoring strict-schema outputs",
    )
    args = parser.parse_args()

    evaluate_results(args.results, ground_truth_file=args.ground_truth)
